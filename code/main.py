...
auth = tweepy.OAuthHandler(CK, CS) #API key , API secret key
auth.set_access_token(ATK, ATS) #Access token , Access token secret
api = tweepy.API(auth)
...
if dt_now.hour == 22:
    tweet = '【静岡県東部の明日の天気】\n' + weather + '\n\n最高気温: ' + maxtemp + '℃\n\n降水確率\n午前: ' + rainam + '%\n午後: ' + rainpm + '%'
media_up = api.media_upload(filename = 'img.png', file = result)
api.update_status(status = tweet, media_ids=[media_up.media_id])
print(tweet)